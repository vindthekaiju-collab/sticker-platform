...
/**
 * Bot API iki metotta FARKLI alan adı bekliyor:
 *   createNewStickerSet → `stickers`  (InputSticker DİZİSİ)
 *   addStickerToSet     → `sticker`   (tek InputSticker)
 *
 * Kod ikisine de tekil `sticker` gönderiyordu; Telegram createNewStickerSet'te
 * bunu görmezden gelip "there is no sticker file in the request" diyordu.
 * Kuru çalışmada görülmesi imkânsız bir hataydı — istek hiç atılmıyordu
 * (2026-08-17, ilk gerçek çalıştırma).
 */
function stickerForm(alanlar, dosyaYolu, format, emoji, cogul) {
  ...
  form.append(cogul ? 'stickers' : 'sticker', JSON.stringify(cogul ? [girdi] : girdi));
...
async function setKur(ayar) {
  ...
  for (const [i, adim] of adimlar.entries()) {
    ...
    const form = stickerForm(alanlar, path.join(ayar.klasor, adim.dosya),
      adim.format, adim.emoji, adim.metot === 'createNewStickerSet');
...
async function trendEsitle(ayarlar) {
  const durumDosya = path.join(depo.VERI, 'trend-durum.json');
  ...
  if (plan.setYeniMi && kalanlar.length) {
    ...
    const form = stickerForm(
      ...
      true);   // createNewStickerSet çoğul `stickers` ister
