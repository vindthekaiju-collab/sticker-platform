 ...
 *
 * Token yoksa KURU ÇALIŞMA: ne yapılacağını plan olarak döndürür.
...
async function trendEsitle(ayarlar) {
  ...
  const yeniler = dosyalar.filter(d => !durum.yuklenen.includes(d));
  ...
  if (plan.setYeniMi && kalanlar.length) {
    ...
    const form = stickerForm(
      ...
      path.join(ayarlar.klasor, ilk), ilk.endsWith('.webm') ? 'video' : 'static');
  ...
  for (const dosya of kalanlar) {
    const form = stickerForm(
      ...
      path.join(ayarlar.klasor, dosya), dosya.endsWith('.webm') ? 'video' : 'static');
