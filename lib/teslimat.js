...
function sayfaUret(setId) {
  ...
  /* Dokun→paylaş ızgarası. Üye başına TEK önizleme; kaynak seçimi:
       1. wastickers .webp  — hep 512, statikse statik animasyonluysa animasyonlu
                              webp; <img> içinde oynar, paylaşım blob'u da bundan
       2. telegram statik .webp — wastickers üretilmemişse
     Telegram'ın animasyon çıktısı .webm; <img> onu göstermez, o yüzden
     buraya hiç girmez.

     Eskiden yalnız `tur === 'statik'` süzülüyordu. Tamamı animasyonlu bir
     sette bu KÜMEYİ BOŞ bırakıyordu: alıcı ödemeyi yapıp sayfayı açtığında
     "bir sticker'a dokun" yazısını görüyor ama tek sticker göremiyordu
     (2026-08-16, "Ofis Hayatı" — 15/15 animasyon). */
  const wa = set.ciktilar && set.ciktilar.wastickers;
  const secim = new Map();
  if (telegram) {
    for (const d of telegram.dosyalar) {
      if (d.tur === 'statik') secim.set(d.adayId, 'telegram/' + d.dosya);
    }
  }
  if (wa) {
    for (const d of wa.dosyalar) secim.set(d.adayId, 'wastickers/' + d.dosya);
  }
  const onizleme = [...secim.values()]
    .map(yol => `<button class="stk" data-src="${yol}" type="button"><img src="${yol}" alt="" loading="lazy"></button>`)
    .join('');
  ...
  const klasor = path.join(depo.CIKTI, set.id);
