...
function sayfaUret(setId) {
  ...
  if (wastickers && wastickers.paket) {
    kartlar.push(`<a class="kanal whatsapp" href="/cikti/${set.id}/wastickers/${wastickers.paket}" download>💬 WhatsApp'a ekle<span>.wastickers indir → "Sticker Maker" ile aç</span></a>`);
  }
  if (zipCikti && zipCikti.paket) {
    kartlar.push(`<a class="kanal zip" href="/cikti/${set.id}/zip/${zipCikti.paket}" download>📦 Paketi indir (ZIP)<span>512px WebP — her yerde kullan</span></a>`);
  }
  if (!kartlar.length) {
  ...
  const sayfa = sablon
    .replace(/{{SET_ADI}}/g, set.ad)
    .replace(/{{SET_ACIKLAMA}}/g, set.aciklama || '')
