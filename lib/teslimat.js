...
function sayfaUret(setId) {
  ...
  // Önizleme: üretilmiş telegram statiklerinden ilk 8'i.
  const onizleme = telegram
    ? telegram.dosyalar.filter(d => d.tur === 'statik').slice(0, 8)
        .map(d => `<img src="telegram/${d.dosya}" alt="" loading="lazy">`).join('')
    : '';
