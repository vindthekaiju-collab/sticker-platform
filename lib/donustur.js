...
/**
 * Temsili karenin ne kadarı opak? 0..1.
 *
 * Neredeyse boş bir sticker sohbette "gönderilmemiş" gibi görünür. Bunu
 * üretim sırasında ölçüp uyarmak, teslimattan sonra fark etmekten ucuz —
 * 2026-08-16'da "Sevimli" setinde %3 dolulukta bir sticker teslimat
 * sayfasında boş kutu olarak çıktı.
 */
async function doluluk(dosyaYolu) {
  const sayfa = await temsiliKare(dosyaYolu);
  const { data, info } = await sharp(dosyaYolu, { page: sayfa })
    .ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  let opak = 0;
  for (let i = 3; i < data.length; i += 4) if (data[i] > 16) opak++;
  return opak / (info.width * info.height);
}
...
module.exports = {
  ffmpegVar, girdiOku, boyutaSigdir, temsiliKare, doluluk,
