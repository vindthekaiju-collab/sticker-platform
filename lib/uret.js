...
function ciktiKlasoru(setId, hedef) {
  const k = path.join(depo.KOK, 'cikti', setId, hedef);
...
async function uyeleriHazirla(set) {
  ...
  for (const adayId of set.uyeler) {
    ...
    try {
      ...
      const tam = path.join(depo.KOK, guncel.dosya);
      const bilgi = await donustur.girdiOku(tam);
      uyeler.push({ adayId, dosya: tam, animasyonlu: bilgi.animasyonlu, sonuc: 'hazir' });
    } catch (e) {
...
async function telegramUret(set) {
  ...
  const rapor = { hedef: 'telegram', dosyalar: [], hatalar: [], zaman: new Date().toISOString() };
  ...
  for (const u of uyeler) {
    ...
    try {
      if (u.animasyonlu) {
        ...
        rapor.dosyalar.push({ adayId: u.adayId, dosya: path.basename(cikti), tur: 'video' });
      } else {
        ...
        rapor.dosyalar.push({ adayId: u.adayId, dosya: path.basename(cikti), tur: 'statik' });
      }
    } catch (e) {
...
async function wastickersUret(set) {
  ...
  const rapor = { hedef: 'wastickers', dosyalar: [], hatalar: uyeler.filter(u => u.sonuc !== 'hazir'), zaman: new Date().toISOString() };
  ...
  girdiler.push({
    ...
    veri: Buffer.from(JSON.stringify({
      ...
      stickers: rapor.dosyalar.map(d => ({ image_file: d.dosya, emojis: ['🙂'] }))
    }, null, 2), 'utf8')
  });
...
async function zipUret(set) {
  ...
  const uyeler = await uyeleriHazirla(set);
  const rapor = { hedef: 'zip', dosyalar: [], hatalar: [], zaman: new Date().toISOString() };
