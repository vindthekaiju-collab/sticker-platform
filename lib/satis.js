...
function tokenAc(token) {
  ...
  // Sayfa yoksa üret; göreli linkler /cikti/<set>/ altına baktığı için
  // <base> etiketi enjekte edilir — sayfa /t/ altından da doğru çalışır.
  const dosya = path.join(depo.KOK, 'cikti', kayit.setId, 'teslimat.html');
  if (!fs.existsSync(dosya)) teslimat.sayfaUret(kayit.setId);
...
module.exports = { tokenUret, tokenAc, gumroadIsle, liste };
