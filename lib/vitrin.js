...
async function vitrinUret(setId) {
  ...
  if (fs.existsSync(kaynakKlasor)) {
    for (const d of fs.readdirSync(kaynakKlasor).filter(d => d.endsWith('.webp')).sort()) {
      kaynaklar.push({ yol: path.join(kaynakKlasor, d), sayfa: 0 });
  ...
  for (const uyeId of set.uyeler) {
    ...
    if (fs.existsSync(tam)) kaynaklar.push({ yol: tam, sayfa: 0 });
  }
  if (!kaynaklar.length) {
  ...
  for (const [i, kaynak] of kaynaklar.entries()) {
    // page:0 — animasyonluysa ilk kare, statikse görselin kendisi.
    const kucuk = await sharp(kaynak.yol, { page: kaynak.sayfa })
