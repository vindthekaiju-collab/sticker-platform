...
/** .wastickers paketini siteye kopyalar; yoksa null. */
function paketiKopyala(set) {
  const rapor = (set.ciktilar && set.ciktilar.wastickers) || {};
  if (!rapor.paket) return null;
  const kay = path.join(depo.CIKTI, set.id, 'wastickers', rapor.paket);
...
function kanalDugmeleri(set, sinif) {
  /* Kullanıcı bulgusu (2026-08-17): "satın almak istersek uygulama
     seçemiyoruz". Önizleme tek başına ürün değil — hangi uygulamaya
     ekleneceği seçilebilmeli. Telegram linki gerçek (bot kurdu),
     WhatsApp .wastickers indirmesi. Satın alma fiyat kararını bekliyor. */
  const d = [];
  if (set.telegramUrl) {
    d.push(`<a class="kanal tg ${sinif}" href="${kacar(set.telegramUrl)}"
        target="_blank" rel="noopener" onclick="event.stopPropagation()">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M21.9 4.3 18.8 19c-.2 1-.9 1.3-1.7.8l-4.8-3.5-2.3 2.2c-.3.3-.5.5-1 .5l.4-4.9L18.2 6c.4-.3-.1-.5-.6-.2L6.6 12.7l-4.8-1.5c-1-.3-1-1 .2-1.5l18.7-7.2c.9-.3 1.6.2 1.2 1.8z"/></svg>
        Telegram'a ekle</a>`);
  }
  if (set.wastickersYol) {
    d.push(`<a class="kanal wa ${sinif}" href="${kacar(set.wastickersYol)}" download
        onclick="event.stopPropagation()">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a10 10 0 0 0-8.6 15L2 22l5.2-1.4A10 10 0 1 0 12 2zm5.8 14.1c-.2.7-1.2 1.3-2 1.4-.5.1-1.2.1-3.8-.9-3.2-1.3-5.2-4.6-5.4-4.8-.1-.2-1.2-1.6-1.2-3s.7-2.1 1-2.4c.3-.3.6-.4.8-.4h.6c.2 0 .4 0 .7.5l.9 2.2c.1.2.1.4 0 .6l-.4.6-.3.3c-.1.1-.3.3-.1.6.2.3.8 1.3 1.7 2.1 1.2 1 2.1 1.4 2.4 1.5.3.1.4.1.6-.1l.9-1c.2-.2.4-.2.6-.1l2.1 1c.3.1.5.2.5.3.1.2.1.7-.1 1.4z"/></svg>
        WhatsApp'a ekle</a>`);
  }
  return d.join('');
}
...
<div id="onizleme" class="onizleme gizli" role="dialog" aria-modal="true" aria-label="Set önizleme">
  ...
  <div class="onizleme-govde">
    <header class="onizleme-ust">
      <h3 id="oz-ad"></h3>
      <p id="oz-alt"></p>
      <div class="oz-kanallar" id="oz-kanallar"></div>
    </header>
...
function setAc(i) {
  ...
  document.getElementById('oz-kanallar').innerHTML = aktifSet.kanallar || '';
